    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-1 glass-card p-6 flex flex-col space-y-4 max-h-[600px] overflow-y-auto">
        <h3 className="font-bold text-lg text-white mb-2 flex items-center">
          <Loader2 className={`mr-2 h-5 w-5 text-cyan-400 ${status === 'running' ? 'animate-spin' : ''}`} />
          Multi-Agent Pipeline
        </h3>
        
        <div className="relative border-l-2 border-slate-700/60 ml-3 pl-6 space-y-6">
            ...
            return (
              <div key={agent} className="relative flex flex-col items-start">
                ...
                <div className="absolute -left-[35px] top-1">
                  ...
                  {agentStatus === 'running' && (
                    <div className="bg-cyan-500/20 text-cyan-400 p-1.5 rounded-full border border-cyan-500/30 animate-pulse">
                      <Loader2 className="h-4 w-4 animate-spin" />
                    </div>
                  )}
                  {agentStatus === 'failed' && (
                    <div className="bg-rose-500/20 text-rose-400 p-1.5 rounded-full border border-rose-500/30">
                      <AlertCircle className="h-4 w-4" />
                    </div>
                  )}
                  {agentStatus === 'pending' && (
                    <div className="bg-slate-800 text-slate-500 p-1.5 rounded-full border border-slate-700">
                      <Hourglass className="h-4 w-4" />
                    </div>
                  )}
                </div>

                <div className={`pl-2 ${agentStatus === 'running' ? 'text-cyan-400 font-bold' : agentStatus === 'completed' ? 'text-slate-300' : 'text-slate-500'}`}>
                  <span className="text-sm block font-semibold">{agent}</span>
                  {agentStatus === 'running' && (
                    <span className="text-xs text-cyan-400/80 animate-pulse">Executing code-generation models...</span>
                  )}
                  {agentStatus === 'completed' && (
                    <span className="text-xs text-emerald-400/80">Documentation generated</span>
                  )}
                </div>
              </div>
            );

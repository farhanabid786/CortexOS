    ...
    <div className="min-h-screen flex flex-col bg-darkBg text-slate-100">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-6 md:px-12 py-10 space-y-8">
        ...
        <div className="flex items-center space-x-4">
          <div className="bg-gradient-to-tr from-cyan-400 to-indigo-600 p-3 rounded-2xl text-white shadow-lg">
            <BrainCircuit className="h-6 w-6" />
          </div>
          <div>
            <h1 className="text-2xl font-extrabold text-white">Blueprint Design Wizard</h1>
            <p className="text-slate-400 text-xs mt-1">Specify parameters to launch the multi-agent software engineering grid.</p>
          </div>
        </div>

        {error && (
          <div className="p-4 rounded-xl border bg-rose-500/10 border-rose-500/20 text-rose-300 text-xs flex items-start space-x-3">
        ...
        <form onSubmit={handleLaunch} className="glass-card p-8 space-y-6">
          <div className="space-y-1">
            <label className="text-[10px] text-slate-400 font-mono tracking-wider block">PROJECT WORKSPACE TITLE *</label>
            <input
              type="text"
              required
              minLength={3}
          ...
          <div className="space-y-1">
            <label className="text-[10px] text-slate-400 font-mono tracking-wider block">SHORT SUBTITLE DESCRIPTION</label>
            <input
              type="text"
              maxLength={500}
              value={description}
          ...
          <div className="space-y-1">
            <label className="text-[10px] text-slate-400 font-mono tracking-wider block">DETAILED SOFTWARE IDEA SPECIFICATION *</label>
            <textarea
              required
              minLength={10}
              rows={6}
            ...
            />
            <span className="text-[10px] text-slate-500 block italic">Minimum 10 characters required. Scanned for safety bypass/jailbreak attempts.</span>
          </div>
          ...
          <div className="pt-4 border-t border-slate-800/80 flex justify-end">
            <button
              type="submit"
              disabled={submitting}
              className="bg-gradient-to-r from-cyan-400 to-indigo-500 hover:from-cyan-500 hover:to-indigo-600 text-slate-950 font-extrabold px-8 py-3.5 rounded-xl flex items-center justify-center space-x-2 transition-all disabled:opacity-50"
            >
